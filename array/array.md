...
#### Sliding Window
- 배열 문제 중 연속된 부분 배열에 대한 문제를 풀어야 할 때 사용하는 경우가 많음.
- Fixed-size 인 경우
    - 윈도우 크기가 고정되어 있으므로, 윈도우 크기가 완성되는 그 순간까지만 잘 처리해주면 됨.
    - e.g., 길이가 m인 연속 부분 배열 중에서 최대값/최소값 찾기
- Variable-size 인 경우
    - 윈도우의 크기가 유동적으로 변해야 하며, 보통 투 포인터나 deque 등을 사용한다는 듯
    - e.g., 합이 k 이상인 가장 짧은 연속 부분 배열의 길이 구하기, 중복 없는 가장 긴 부분 문자열의 길이 구하기, 길이 제한 없이 조건을 만족하는 최소/최대 구간 찾기 등

#### Kadane's Algorithm
- 카데인 알고리즘도 슬라이딩 윈도우와 비슷하게 최대 부분 배열의 합 문제에 사용함.
- 이건 현재까지 누적한 부분합만 갱신하면서 진행하면 되므로 좀 더 간단함.